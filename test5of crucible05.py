...
warning = tk.Label(text='ALL FIELDS MUST BE COMPLETED \nINCLUDING DROPDOWN MENU')
...
#TODO Funtion has bug that only checks conditions when selectedOE is called.  I believe only way to fix is object oriented programming                
def callback(selectedOE):
    ...
    if selectedOE != 'Select the main fault for billable event':
        ...
        for entry in entry_list:
            if entry.get():
                warning.config(bg='GREEN')
                warning.config(text="ALL REQUIRED FIELDS COMPLETED\nDON'T FORGET PARTS")
